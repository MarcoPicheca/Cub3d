...
se il movimento e' possibile aggiorna la posizione altrimenti ritorna al chiamante.
divido per block per passare dalle coordinate di mondo alle coordinate della griglia.
la condizione controlla che non ci si avvicini troppo al muro nelle 4 direzioni,
se non siamo troppo vicini aggiorna la posizione
*/
void update_position(t_play *player, t_map *map, float move_x, float move_y)
{
    float new_x;
    float new_y;
    float radius; // Raggio attorno al giocatore
...
void move_player(t_play *player, t_map *map)
{
    ...
    if (player->key_up)
        update_position(player, map, cos_angle * (*move_speed), sin_angle * (*move_speed));
    if (player->key_down)
        update_position(player, map, -cos_angle * (*move_speed), -sin_angle * (*move_speed));
    if (player->key_left)
        update_position(player, map, sin_angle * (*move_speed), -cos_angle * (*move_speed));
    if (player->key_right)
        update_position(player, map, -sin_angle * (*move_speed), cos_angle * (*move_speed));
}