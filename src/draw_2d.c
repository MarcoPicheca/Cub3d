...
/*   Updated: 2025/01/08 02:09:44 by tschetti         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*
disegna raggio per raggio (a partire da sinistra), ognuno fino al raggiungimento della distanza
aggiorna via via i coefficienti di avanzamento per colorare i pixel che lo rappresentano
*/
void draw_rays(t_game *game, t_render_2d *values)
{
    ...
    while (i < values->num_rays)
    {
        ...
        while (j < (int)rays.distance)
        {
            m = (int)rays.cx;
            n = (int)rays.cy;
            // if (m >= 0 && m < WIN_WIDTH && n >= 0 && n < WIN_HEIGHT)
...
start_x e start_y coordinate d'inizio per rendering del rettangolo (moltiplico per loro fattore di scala)
width e height grandezza del rettangolo
*/
void draw_map(t_game *game, t_render_2d *values)
{
    ...
    while (y < game->map.height)
    {
        ...
        while (x < game->map.width)
        {
            ...
            // else if(game->map.mtx2[y][x] == '0' || game->map.mtx2[y][x] == 'S' || ...)
            // {
            //     params.start_x = x * values->scale_x;
            //     params.start_y = y * values->scale_y;
            //     params.width = values->scale_x;
            //     params.height = values->scale_y;
            //     params.color = 0x33EEFF;
            //     draw_rectangle(&params, game);   
            // }
            x++;
        }
