...
un bordo verticale(2d) di una cella(continuera a muoversi lungo y), 1e30 e' un approssimazione
a infinito (stesso discorso per componente y), altrimenti (se non e' 0) viene
calcolata come il reciproco del coseno(o seno) della direzione del raggio [piu inclinazione piu tempo/distanza].
...
void init_ray_params(t_ray_cast_params *params, t_game *game, float angle)
{
    ...
    {
        params->step_x = -1;
        params->side_dist_x = (game->player.x - params->map_x) * params->delta_dist_x;
    }
    else
    {
        params->step_x = 1;
        params->side_dist_x = ((params->map_x + 1.0f) - game->player.x) * params->delta_dist_x;
    }
    if (params->ray_dir_y < 0)
    {
        params->step_y = -1;
        params->side_dist_y = (game->player.y - params->map_y) * params->delta_dist_y;
    }
    else
    {
        params->step_y = 1;
        params->side_dist_y = ((params->map_y + 1.0f) - game->player.y) * params->delta_dist_y;
    }
}
...
void calculate_hit_details(t_ray_cast_params *params, t_game *game)
{
    if (params->side_local == 0)
        params->perp_wall_dist = (params->map_x - game->player.x + (1 - params->step_x) / 2) / params->ray_dir_x;
    else
        params->perp_wall_dist = (params->map_y - game->player.y + (1 - params->step_y) / 2) / params->ray_dir_y;

    params->hit_x = (game->player.x + params->perp_wall_dist * params->ray_dir_x) * BLOCK;
